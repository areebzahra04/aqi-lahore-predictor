...
def check_csv_exists():
    if not os.path.exists(INPUT_CSV):
        print("Error: Training data CSV not found.")
        print("Please run export_training_data.py first to generate the CSV.")
        return False
    return True
...
def load_and_prepare_data():
    ...
    if len(df) < 24:
        print(f"Insufficient data: {len(df)} rows. Need at least 24 rows for training.")
        return None, None, None, None
    
    split_idx = int(len(df) * 0.8)
    
    if split_idx < 10:
        print(f"Not enough samples for train/test split: {split_idx} training samples")
        return None, None, None, None
    
    X = df[FEATURE_COLS].values
    X_train_full, X_test_full = X[:split_idx], X[split_idx:]
    
    return df, X_train_full, X_test_full, split_idx
...
def main():
    ...
    df, X_train_full, X_test_full, split_idx = result
    ...
    print("\n3. Training TensorFlow models...")
    for horizon in HORIZONS:
        print(f"\n   Training {horizon} forecast model...")
        y = df[f"target_{horizon}"].values.reshape(-1, 1)
        y_train_raw, y_test_raw = y[:split_idx], y[split_idx:]
