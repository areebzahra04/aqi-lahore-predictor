...
def connect_hopsworks():
    try:
        api_key = get_api_key()
        if not api_key:
            st.error("No API key found. Please set HOPSWORKS_API_KEY in .env file or Streamlit secrets.")
            return None, None, None
...
def load_live_data(_fs, hours=200):
    """Load LIVE data from Hopsworks Feature Store"""
    try:
        fg = _fs.get_feature_group(name="aqi_lahore_features", version=3)
        ...
        if "Set changed size during iteration" in error_msg:
            ...
            return None, 0, f"Error reading data: {error_msg}"
...
def main():
    ...
    with st.spinner("Connecting to feature store..."):
        ...
        # Load LIVE data from Hopsworks
        df, record_count, error_msg = load_live_data(fs)
        ...
        st.sidebar.write(f"Total records: {record_count}")
        st.sidebar.write(f"Latest timestamp: {df['timestamp'].max()}")
